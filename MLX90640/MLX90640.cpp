...
uint16_t MLX90640::whoIAM()
{
    uint16_t id = 0;
    readRegs(0x2407, (uint8_t *)&id, sizeof(id));
    return id;
}

void MLX90640::DumpEE(uint16_t eedata[832])
{
    readRegs(0x2400, (uint8_t*)eedata, 832*sizeof(uint16_t));
}

int MLX90640::init(uint16_t buff[832])
{
    int error = 0;
    DumpEE(buff);
    ExtractVDDParameters(buff, &params);
    ExtractPTATParameters(buff, &params);
    ExtractGainParameters(buff, &params);
    ExtractTgcParameters(buff, &params);
    ExtractResolutionParameters(buff, &params);
    ExtractKsTaParameters(buff, &params);
    ExtractKsToParameters(buff, &params);
    ExtractCPParameters(buff, &params);
    ExtractAlphaParameters(buff, &params);
    ExtractOffsetParameters(buff, &params);
    ExtractKtaPixelParameters(buff, &params);
    ExtractKvPixelParameters(buff, &params);
    ExtractCILCParameters(buff, &params);
    error = ExtractDeviatingPixels(buff, &params);      
    return error;
}

int MLX90640::getFrameData(uint16_t frameData[834])
{
    ...
    while(dataReady == 0)
    {
        error = readRegs(0x8000, (uint8_t*)&statusRegister, sizeof(statusRegister));
        if(error != 0)
        {
            return error;
        }        
    ...
    { 
        error = writeRegs(0x8000, 0x0030);
        if(error == -1)
        {
            return error;
        }

        error = readRegs(0x0400, (uint8_t*)frameData, sizeof(uint16_t) * 832);
        if(error != 0)
        {
            return error;
        }
    ...
    error = readRegs(0x800D, (uint8_t*)&controlRegister1, sizeof(controlRegister1));
...
int MLX90640::writeRegs(uint16_t addr, uint16_t data) const
{
    char cmd[4] =  { (char)(addr >> 8), (char)(addr & 0xFF), 
                     (char)(data >> 8), (char)(data & 0xFF)
                   };
    return _i2c->write(_addr, cmd, 4);
}

int MLX90640::readRegs(uint16_t addr, uint8_t *data, uint32_t len) const
{
    int status = 0;
    char t[2] = { (char)(addr >> 8), (char)(addr & 0xFF)};
    _i2c->write(_addr, t, sizeof(t), true);
    status = _i2c->read(_addr|1, (char *)data, len);
    // for little endbian/cortex-m
...
//------------------------------------------------------------------------------
int CheckAdjacentPixels(int16_t pix1, int16_t pix2)
{
    int pixPosDif;
